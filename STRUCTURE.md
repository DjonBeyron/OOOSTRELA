...
- `src/lib/defaultKnowledge.ts` — база знаний о компании по умолчанию (услуги, адрес, история, директор).
- `src/lib/brandFilter.ts` — замена названий реальной нейросети/разработчика/движка на «Машинный интеллект» в журнале админки.
- `src/lib/emoji.ts` — вырезание и подсчёт эмодзи в ответе модели (галочка «Без смайликов» в админке).
- `src/lib/thoughtCleaner.ts` — чистка хода мысли для журнала: вырезает фразы, где модель проговаривает скрытые правила.
- `src/lib/stages.ts` — лента этапов обработки запроса для журнала (вход → фильтр → правила → размышление → фильтр выхода → ответ).
...
- `src/lib/moderation.ts` — разбор вопроса по темам (слои 1–2) и скрытая инструкция модели: кто она, база знаний о компании, запреты, заготовка.
...
- `src/features/admin/RequestsTab.tsx` — журнал запросов: список и кнопка «Обновить».
- `src/features/admin/RequestCard.tsx` — карточка запроса: лента этапов, ход мысли по шагам, ответ.
- `src/features/admin/RulesTab.tsx` — темы: переключатели проверок, база знаний о компании, список тем, «Сохранить».
