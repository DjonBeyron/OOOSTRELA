...
//   thinking — модель думает: стрелка смотрит вправо и «натягивается», как тетива, рядом подпись.
//   writing  — модель пишет: стрелка плавно поворачивается вниз и пульсирует вниз, подпись уходит.
...
export default function TypingArrow({ mode }: { mode: 'thinking' | 'writing' }) {
  return (
    <span className={`typing-arrow is-${mode}`} role="status">
      ...
      {mode === 'thinking' ? (
        <span className="typing-arrow-label">Машинный интеллект думает…</span>
      ) : (
        <span className="visually-hidden">Пишет ответ</span>
      )}
    </span>
  )
}
